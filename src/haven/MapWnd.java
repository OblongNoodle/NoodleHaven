...
public class MapWnd extends Window implements Console.Directory {
	...
	private class ViewFrame extends Frame {
		...
		public boolean mousedown(MouseDownEvent ev) {
			...
			// Only check compact mode resize handle
			if((ev.b == 1) && MapWnd.this.compact() && (cc.x < sizer.sz().x) && (cc.y < sizer.sz().y) && (cc.y >= sizer.sz().y - UI.scale(25) + (sizer.sz().x - cc.x))) {
				if(drag == null) {
	...
	public void resize(Coord sz) {
		sz = sz.max(compact() ? UI.scale(150, 150) : UI.scale(250, 180));
		super.resize(sz);

		// Leave space for resize handle at bottom in non-compact mode
		int availableHeight = compact() ? sz.y : sz.y - UI.scale(15);

		tool.resize(availableHeight);
		if(!compact()) {
